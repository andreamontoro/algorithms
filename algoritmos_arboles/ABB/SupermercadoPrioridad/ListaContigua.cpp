...
ListaContigua::ListaContigua() {
	vector = NULL; // No hay que reservar memoria ya que la ListaContigua se crea vacia en un principio
	n=0;
	capacidad=0;
}
...
void ListaContigua::concatenar(ListaContigua *listaAConcatenar) {
	...
	// Tamano de la lista a concatenar. Por convencion, cuando tenemos
	// dos listas, el tamano de una es "n" y el de la otra es "m"
	...
	// capacidad en el tamano de la listaAConcatenar
	// Complejidad temporal O(n) 
	// Complejidad espacial O(m+n)
	vector = (int*)realloc(vector, sizeof(int)*(capacidad + m));
	capacidad += m;

	// Vamos insertando al final elemento a elemento. Fijate en que insertar un elemento al final
	// es O(1) si no tenemos que ampliar la capacidad (la cual ya esta ampliada)
...
int ListaContigua::buscar(int elementoABuscar) {
	int posicion = 0; // Posicion actual en donde buscamos
	...
	// Devolvemos la posicion si lo hemos encontrado, o -1 en caso contrario
